...
async function connectDB() {
  ...
  try {
    await mongoose.connect(catalogDb1Uri, {
      autoIndex: process.env.NODE_ENV !== 'production',
      serverSelectionTimeoutMS: 15000
    });
    logger.info('MongoDB default mongoose connection (DB1 catalog_db_1) ready');
  } catch (error) {
    logger.error(`MongoDB DB1 connection failure: ${error.message}`);
    process.exit(1);
  }
}
